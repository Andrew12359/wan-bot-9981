...
if (!opts['test']) setInterval(() => {
  ...
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('Database gagal di save ❌')
  else {
    global.DATABASE.save()
    conn.logger.info('Database siap di save ⌛')
    ...
    let user = global.DATABASE._data.users[m.sender]
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < m.limit * 1 && plugin.limit) {
          this.reply(m.chat, `Limit anda sudah habis, sila beli dengan menaip *${usedPrefix}buy*`, m)
        ...
        try {
          ...
          if (m.limit) m.reply(+ m.limit + '1 limit dipakai, kerana fitur ini terhad')
        }
  			break
  		}
  	}
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg: 'Anda belum mendaftar di robot saya, sila daftar dengan cara menaip:\n\n#daftar <nama>.<umur>\n\nContoh:\n#daftar Wan.13'
  }[type]
